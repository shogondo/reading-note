...
* なぜ Pod が必要なのか？コンテナを直接使ってはいけないのか？なぜ複数コンテナを実行する必要があるのか？1 コンテナで複数のプロセスを実行してはいけないのか？
  ...
  * 1 コンテナで複数のプロセスを実行すると、それらのプロセスを実行し続ける、ログをどう管理するか、といった点を考慮する必要が出てくる
...
* **Pod 内の全コンテナは同じネットワーク、UTS 名前空間で稼働するので、ホスト名、ネットワークインターフェースを共有する**
* **IPC 名前空間も共有するので IPC 通信可能**
...
* **各 Pod はフラットな共有ネットワークネームスペース内で IP アドレスを持ち、お互いに IP アドレスで通信可能 (No NAT)**
...
* ドキュメント http://kubernetes.io/docs/api の他、`kubectl explain` コマンドで属性の説明を参照することが可能。
...
* マニフェストファイルを使った Pod の作成

```
$ kubectl create -f <マニフェストファイル>
```

* `kubectl get pods` で Pod の一覧を表示。
...
* リソース名は Namespace の中でユニークにしなければならないので、別の Namespace であれば同じリソース名を使える
